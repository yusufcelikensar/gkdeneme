...
# AdminPanel sınıfı içinde:
    ...
    def show_edit_member_form(self, member_id):
        ...
        # Gerekli widget'lar oluşturulmuş mu kontrol et
        required_widgets = [self.edit_name_input, self.edit_uid_input, self.edit_department_input,
                            self.edit_year_input, self.edit_interests_input, self.edit_email_input,
                            self.edit_phone_input, self.edit_role_combo, self.edit_photo_input,
                            self.edit_membership_date_edit, self.edit_photo_label]
        if any(widget is None for widget in required_widgets):
             QMessageBox.critical(self, "Hata", "Üye düzenleme formu elemanları henüz tam olarak yüklenmemiş.")
             return

        self.edit_member_id = member_id # Düzenlenecek ID'yi sakla
        try:
            cursor = self.get_cursor()
            cursor.execute("SELECT * FROM members WHERE id = %s", (member_id,))
            ...
            if member:
                ...
                self.edit_name_input.setText(member['name'])
                self.edit_uid_input.setText(member['uid'])
                self.edit_department_input.setText(member['department'] or "")
                self.edit_year_input.setText(str(member['year']) if member['year'] is not None else "")
                self.edit_interests_input.setText(member['interests'] or "")
                self.edit_email_input.setText(member['email'] or "")
                self.edit_phone_input.setText(member['phone'] or "")
                self.edit_photo_input.setText(member['photo_path'] or "")

                # Rol ComboBox'ını ayarla
                role = member['role'] or ""
                role_index = self.edit_role_combo.findText(role)
                self.edit_role_combo.setCurrentIndex(role_index if role_index >= 0 else 0) # Bulamazsa ilkini seç

                # Tarihi ayarla
                date_str = member["membership_date"] or ""
                qdate = QDate.fromString(date_str, Qt.DateFormat.ISODate)
                self.edit_membership_date_edit.setDate(qdate if qdate.isValid() else QDate.currentDate())

                # Fotoğrafı yükle
                photo_db_filename = member['photo_path'] # Veritabanından gelen SADECE dosya adı
                self.load_member_photo_to_label(photo_db_filename, self.edit_photo_label)

                # Düzenleme sayfasını göster
                self.stacked_widget.setCurrentWidget(self.edit_member_page)
            else:
                QMessageBox.warning(self, "Hata", f"ID'si {member_id} olan üye bulunamadı.")
                self.edit_member_id = None # ID'yi sıfırla

        except psycopg2.Error as e:
            QMessageBox.critical(self, "Veritabanı Hatası", f"Üye bilgileri yüklenirken hata: {e}")
            self.edit_member_id = None
        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Üye bilgileri yüklenirken beklenmedik hata: {e}")
            self.edit_member_id = None
            import traceback
            traceback.print_exc()
...
# AdminPanel sınıfının içinde:
    def record_attendance(self, member_id, event_id, status_label_ref):
        """Verilen üye ve etkinlik için katılım kaydı oluşturur, puan ekler ve loglar."""
        try:
            cursor = self.get_cursor()
            timestamp = QDateTime.currentDateTime().toString(Qt.DateFormat.ISODate) # Katılım zamanı

            # 1. Önce katılımı kaydet ve commit et
            cursor.execute("INSERT INTO attendance (member_id, event_id, timestamp) VALUES (%s, %s, %s)",
                        (member_id, event_id, timestamp))
            self.db_connection.commit() 
            print(f"Katılım kaydedildi (DB'ye commit edildi): Üye ID {member_id}, Etkinlik ID {event_id}")

            # 2. Şimdi puan eklemeyi ve loglamayı dene (ayrı bir try-except içinde)
            points_awarded_successfully = False # Başlangıçta puan eklenmedi varsayalım
            points_to_award = ATTENDANCE_POINTS # Üstte tanımladığımız sabit
            
            try:
                # Puan logu için etkinlik adını alalım
                cursor.execute("SELECT name FROM events WHERE id = %s", (event_id,))
                event_row = cursor.fetchone()
                event_name = event_row['name'] if event_row else f"ID:{event_id}" # Etkinlik silinirse diye ID'yi yaz
                reason_text = f"Etkinlik Katılımı: {event_name}"
                log_timestamp = QDateTime.currentDateTime().toString(Qt.DateFormat.ISODate) # Log zamanı
                ...
                self.db_connection.commit() # Puan güncelleme ve loglamayı commit et
                print(f"DEBUG: Üye {member_id} için katılım puanı ({points_to_award}) eklendi ve loglandı.")
                points_awarded_successfully = True

            except psycopg2.Error as e_points_db:
                # Puan ekleme başarısız olursa, katılım zaten kaydedildi. Sadece logla ve uyar.
                print(f"HATA: Katılım kaydedildi ancak puan eklenirken/loglanırken DB hatası: {e_points_db}")
                # Burada veritabanını geri almaya (rollback) gerek yok, çünkü katılımın kaydedilmesi daha önemli olabilir.
            except Exception as e_points_gen:
                print(f"HATA: Katılım kaydedildi ancak puan eklenirken/loglanırken genel hata: {e_points_gen}")
                traceback.print_exc()

            # 3. Başarı mesajını ayarla (puan bilgisini de içerecek şekilde)
            ...
            else: # Eğer puan ekleme başarısız olduysa
                success_message += f"<br><font color='orange'>Puan eklenirken bir sorun oluştu.</font>"
            status_label_ref.setText(success_message)
            
            return True # Katılım işlemi başarılı (puan eklenemese bile)

        except psycopg2.Error as e_integrity:
            # Bu kısım üyenin zaten katılmış olma durumunu ele alır (öncekiyle aynı)
            if "UNIQUE constraint failed: attendance.member_id, attendance.event_id" in str(e_integrity):
                try: # Üye adını alıp mesaj vermek için
                    cursor = self.get_cursor()
                    cursor.execute("SELECT name FROM members WHERE id=%s", (member_id,))
                    mem_name_row = cursor.fetchone()
                    mem_name = mem_name_row['name'] if mem_name_row else 'Bilinmeyen Üye'
                    ...
                    mem_name = 'Bu üye'
                status_label_ref.setText(f"<font color='blue'>'{mem_name}' bu etkinliğe zaten katılmış.</font>")
                print(f"Üye {member_id} zaten etkinlik {event_id}'ye katılmış.")
                return False 
            else: # Başka bir IntegrityError (örn: olmayan üye/etkinlik ID'si)
                status_label_ref.setText(f"<font color='red'><b>Veritabanı Hatası (Integrity):</b> {e_integrity}</font>")
                print(f"Katılım kaydedilirken IntegrityError: {e_integrity}")
                traceback.print_exc()
                return False
        except psycopg2.Error as e_db_main:
            status_label_ref.setText(f"<font color='red'><b>Veritabanı Hatası:</b> {e_db_main}</font>")
            print(f"Katılım kaydedilirken DB Hatası: {e_db_main}")
            traceback.print_exc()
            return False
        except Exception as e_gen_main:
            status_label_ref.setText(f"<font color='red'><b>Beklenmedik Hata:</b> {e_gen_main}</font>")
            print(f"Katılım kaydedilirken genel hata: {e_gen_main}")
            traceback.print_exc()
            return False
